...
public class WebSphereDeploymentService extends AbstractDeploymentService {
    ...
    public void installArtifact(Artifact artifact) {
        ...
        try {        	
            ...
            DeploymentNotificationListener notifyListener = new DeploymentNotificationListener(getAdminClient(), filterSupport, "Install " + artifact.getAppName(),AppNotification.INSTALL,verbose);
	...
	public void updateArtifact(Artifact artifact) {
        ...
        try {
            NotificationFilterSupport filterSupport = createFilterSupport();
            DeploymentNotificationListener notifyListener = new DeploymentNotificationListener(getAdminClient(), filterSupport, "Update " + artifact.getAppName(),AppNotification.INSTALL,verbose);
    ...
    public void uninstallArtifact(Artifact artifact) throws Exception {
    	try {
			NotificationFilterSupport filterSupport = createFilterSupport();
			DeploymentNotificationListener notifyListener = new DeploymentNotificationListener(getAdminClient(),filterSupport,"Uninstall " + artifact.getAppName(), AppNotification.UNINSTALL,verbose);
    ...
    private boolean waitForApplicationDistribution(AppManagement appManagementProxy,Artifact artifact,int secondsToWait) throws Exception {
		...
		while (checkDistributionStatus(distributionListener) != AppNotification.DISTRIBUTION_DONE && totalSeconds < secondsToWait) {
			Thread.sleep(1000);			
			totalSeconds++;
			distributionListener = new DeploymentNotificationListener(getAdminClient(), filterSupport, null,AppNotification.DISTRIBUTION_STATUS_NODE,verbose);
